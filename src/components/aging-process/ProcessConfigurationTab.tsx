...
const ProcessConfigurationTab = () => {
  ...
  const [newState, setNewState] = useState({ name: '', description: '', pythonScript: '', delay: 0, jumpTarget: 'end', mode: 'script' as 'script' | 'condition' });
...
system.get_state()       # 获取当前状态`;
  ...
  const handleAddState = () => {
    ...
    if (newState.mode === 'script' && !newState.pythonScript.trim()) {
      showError('请输入Python脚本');
      return;
    }
    ...
    const state: ProcessState = {
      ...
      pythonScript: newState.mode === 'script' ? newState.pythonScript : '',
      delay: newState.delay,
      conditions: newState.mode === 'condition' ? [] : [],
      jumpTarget: newState.jumpTarget,
      mode: newState.mode
    };

    setProcessStates([...processStates, state]);
    setNewState({ name: '', description: '', pythonScript: '', delay: 0, jumpTarget: 'end', mode: 'script' });
    showSuccess('状态添加成功');
  };
  ...
  const handleAddCondition = (stateId: string) => {
    ...
    setProcessStates(processStates.map(state => 
      state.id === stateId && state.mode === 'condition'
        ? { 
            ...state, 
              ...
              <Card>
                ...
                <CardContent className="space-y-6">
                  ...
                  <div className="space-y-4 p-4 border rounded-lg">
                    ...
                    <div className="flex space-x-4 mb-4">
                      <Button
                        variant={newState.mode === 'script' ? "default" : "outline"}
                        onClick={() => setNewState({...newState, mode: 'script'})}
                        className="flex-1"
                      >
                        <Circle className={`mr-2 h-4 w-4 ${newState.mode === 'script' ? 'text-white' : 'text-muted-foreground'}`} />
                        Python脚本模式
                      </Button>
                      <Button
                        variant={newState.mode === 'condition' ? "default" : "outline"}
                        onClick={() => setNewState({...newState, mode: 'condition', pythonScript: ''})}
                        className="flex-1"
                      >
                        <Circle className={`mr-2 h-4 w-4 ${newState.mode === 'condition' ? 'text-white' : 'text-muted-foreground'}`} />
                        条件模式
                      </Button>
                    </div>
                    
                    {newState.mode === 'script' ? (
                      <div className="space-y-2">
                        <Label>Python脚本</Label>
                        <Textarea
                    ...
                    <div className="space-y-4">
                      ...
                      {processStates.map(state => (
                        <div key={state.id} className="border rounded-lg p-4">
                          ...
                          {state.mode === 'script' && state.pythonScript && (
                            <div className="mb-3">
