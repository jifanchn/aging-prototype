...
const WorkstationOverview = () => {
  ...
  const handleCreateWorkstation = (name: string) => {
    const newWorkstation: Workstation = {
      id: workstations.length + 1,
      name,
      status: 'stopped',
      onlineDevices: [],
      logs: [{ timestamp: 0, content: "工位创建成功" }],
      temperature: 25.0,
      voltage: 220,
      uptime: "0h 0m",
      importantPoints: [],
      infoMessage: "工位已创建，等待配置"
    };
    setWorkstations([...workstations, newWorkstation]);
    showSuccess(`工位 ${name} 创建成功`);
  };

  const handleAction = (workstationId: number, action: 'start' | 'stop' | 'fail' | 'delete') => {
    if (action === 'delete') {
      if (window.confirm(`确定要删除工位 ${workstations.find(w => w.id === workstationId)?.name} 吗？删除后无法恢复！`)) {
        setWorkstations(workstations.filter(w => w.id !== workstationId));
        showSuccess('工位删除成功');
      }
      return;
    }
    ...
    setWorkstations(workstations.map(ws => {
      if (ws.id === workstationId) {
        ...
        let newInfoMessage = ws.infoMessage;
        let newLogs = [...ws.logs];
        
        switch (action) {
          case 'start':
            if (ws.status === 'stopped' || ws.status === 'failed') {
              newStatus = 'running';
              newInfoMessage = '工位启动中';
              newLogs.push({ timestamp: Date.now() / 1000, content: "工位启动成功" });
              showSuccess(`工位 ${ws.name} 启动成功`);
            }
            break;
          case 'stop':
            if (ws.status === 'running' || ws.status === 'paused') {
              newStatus = 'stopped';
              newInfoMessage = '工位已停止';
              newLogs.push({ timestamp: Date.now() / 1000, content: "工位停止运行" });
              showSuccess(`工位 ${ws.name} 停止成功`);
            }
            break;
          case 'fail':
            if (ws.status === 'running') {
              newStatus = 'failed';
              newInfoMessage = '工位老化失败';
              newLogs.push({ timestamp: Date.now() / 1000, content: "工位老化测试失败" });
              showSuccess(`工位 ${ws.name} 标记为失败`);
            }
            break;
        }
        
        return { ...ws, status: newStatus, infoMessage: newInfoMessage, logs: newLogs };
      }
      return ws;
    }));
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredWorkstations.map((workstation) => (
          <WorkstationCard
            ...
            infoMessage={workstation.infoMessage}
            onDetailsClick={() => handleViewDetails(workstation)}
            onActionClick={(action) => handleAction(workstation.id, action)}
          />
        ))}
      </div>
